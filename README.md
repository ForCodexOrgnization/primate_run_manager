...
## Task-native pipeline scheduling

`PIPELINE_MODE` selects exactly one canonical scheduler:

```
PIPELINE_MODE=streaming_per_sample
all eligible samples -> one immutable submission -> Slurm array
                     -> one task per sample -> %SAMPLE_CHAIN_CONCURRENCY

PIPELINE_MODE=batch
all eligible samples -> one immutable submission -> launcher batch files
                     -> one task per batch -> %CHAIN_CONCURRENT_BATCHES
```

`legacy_batch` is accepted as an alias and normalized to `batch`. The manager derives
the launcher from `STREAMING_PIPELINE_LAUNCHER` or `BATCH_PIPELINE_LAUNCHER`; the
mode and launcher do not need to be synchronized manually. There is no 50-sample
manager wave scheduling barrier in either mode. `PIPELINE_WAVE_SIZE` and
`MAX_ACTIVE_PIPELINE_WAVES` are deprecated and ignored by task-native scheduling.

A submission is an immutable audit record, not a lifecycle barrier. Exact task
identity is stored in `state/submission_task_map/<submission_id>.tsv`. Streaming
IDs are `1..N`, retain persistent `PIPELINE_WORK_ROOT/<sample>` work directories,
and use one row per sample. Batch IDs are `0..B-1`; mapping is read from the exact
sorted `sample_batch_*` files generated by the launcher, with multiple sample rows
per batch task. Reconciliation queries `sacct -j JOB_TASK`, validates every sample
independently, and can make completed output transfer-ready while sibling tasks
remain queued or running.

Only one submission may own the global concurrency budget at a time. New `PENDING`
samples are submitted before `PIPELINE_DEFERRED_RETRY` samples. Deferred work is
eligible only when pending work and active normal tasks are absent. Existing
terminal/manual/transfer states are not reset, and historical wave records remain
...
`STREAMING_SUBMISSION_WINDOW=0` selects every currently eligible, unowned sample. Streaming arrays may overlap, but new arrays are submitted with `STREAMING_SUBMIT_HELD=1`; the unified exact-element controller composes `DISK_PRESSURE`, `GLOBAL_CONCURRENCY`, and `RESUME_PRIORITY` ownership and releases an element only after all manager reasons clear. Disk holds engage at `WORK_CRITICAL_PERCENT` and remain manager-owned until usage reaches `WORK_ARRAY_RELEASE_PERCENT` or lower. Only an existing unified-ledger row, the `INITIAL_SUBMISSION` handshake, or a migrated legacy disk row proves ownership; unrelated `JobHeldUser` and all `JobHeldAdmin` elements are never adopted or released. The pipeline launcher must honor `STREAMING_SUBMIT_HELD=1` by passing Slurm's held-submission option to `sbatch`.

Resume candidates are placed ahead of fresh work during each lightweight admission reconciliation (normally within `DISK_PRESSURE_POLL_SECONDS`). A timeout worker writes its authoritative marker before self-requeue, but it does not synchronously invoke the admission controller before `scontrol requeue`; consequently, Slurm may start already-eligible fresh work in that interval. The controller never cancels or suspends that running work, so this remains a bounded scheduler race rather than a strict pre-requeue guarantee.
