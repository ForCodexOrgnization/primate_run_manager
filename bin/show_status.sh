...
printf 'Pipeline submission %s\n' "$([[ "$ENABLE_PIPELINE_SUBMIT" == 1 ]] && echo enabled || echo disabled)"
...
if [[ "$PIPELINE_MODE" == streaming_per_sample ]]; then
 ...
 printf 'Ready to transfer: %s\n' "$(awk -F '\t' 'NR>1&&$4=="READY_TO_TRANSFER"{n++}END{print n+0}' "$STATUS_FILE")"
 printf 'Active streaming submissions: %s\n' "$(active_wave_count)"
 printf 'Active Slurm array job IDs: %s\n' "$(awk -F '\t' 'NR>1&&$9~/^(CREATED|SUBMITTED|RUNNING)$/&&$4!=""{a[$4]=1}END{for(j in a)printf "%s%s",sep,j;sep=","}' "$WAVE_STATUS_FILE")"
else
 printf 'Wave size: %s\n' "$PIPELINE_WAVE_SIZE"
